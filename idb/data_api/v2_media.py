...
def respond_to_record(r, deriv=None, format=None):
    if r is not None:
        ...
        if r[1] is not None and r[2] is not None:
            if deriv is None:
                media_url = "https://s.idigbio.org/idigbio-{0}-prod/{1}".format(
                    r[1], r[2])
            elif r[1] in ["images", "sounds"
                         ] and deriv in ["thumbnail", "webview", "fullsize"]:
                if r[5]:  # If derivatives have been generated
                    media_url = "https://s.idigbio.org/idigbio-{0}-prod-{2}/{1}.jpg".format(
                        r[1], r[2], deriv)
...
def lookup_ref(format):
    ...
    params = {}
    for ak, pk in [("filereference", "url"), ("type", "type"),
                   ("prefix", "prefix"), ("user", "owner"),
                   ("mime_type", "mime")]:
...
def upload():
    ...
    if r is None or r[2] == request.authorization.username:
        ...
        h, size = calcFileHash(request.files["file"],
                               op=False,
                               return_size=True)
